...
void Lista<T>::aSegundo(){
    Nodo<T> *aMover = inicio;
    if (aMover == nullptr)     std::cout << "No tenes nodos" << std::endl;
    else{
        Nodo<T> *aux = aMover->getSiguiente();
        if (aux == nullptr)     std::cout << "Tenes 1 solo nodo" << std::endl;
        else{
            aMover->setSiguiente(aux->getSiguiente());
            inicio = aux;
            aux->setSiguiente(aMover);
        }
    }
