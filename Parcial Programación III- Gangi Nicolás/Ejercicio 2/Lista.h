...
private:
    ...
    int sumatoria(int n, Nodo<T> *ini);
...
template<class T>
int Lista<T>::sumatoria(int n){
    return sumatoria(n, inicio);
}

template<class T>
int Lista<T>::sumatoria(int n, Nodo<T> *ini){
    if(ini == nullptr)     return 0;

    if(ini->getDato() > n) return ini->getDato() + sumatoria(n, ini->getSiguiente());

    else return 0 + sumatoria(n, ini->getSiguiente());
}
